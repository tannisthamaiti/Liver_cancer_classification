...
class wdfReader(object):
    def __init__(self, file_name):
        try:
            self.file_size = size = os.stat(file_name).st_size
    ...
    def dump_headers(self):
        print ("point_per_spectrum = {}".format(self.point_per_spectrum ))
        print ("capacity = {}".format(self.capacity ))
        print ("count = {}".format(self.count ))
        print ("accumulation_count = {}".format(self.accumulation_count ))
        print ("ylist_length = {}".format(self.ylist_length ))
        print ("xlist_length = {}".format(self.xlist_length ))
        print ("data_origin_count = {}".format(self.data_origin_count ))
        print ("application_name = {}".format(self.application_name ))
        print ("application_version[i] = {}".format(self.application_version ))
        print ("scan_type = {}".format(self.scan_type ))
        print ("measurement_type = {}".format(self.measurement_type ))
        print ("spectral_units = {}".format(self.spectral_units ))
        print ("laser_wavenumber = {}".format(self.laser_wavenumber ))
        print ("username = {}".format(self.username ))
        print ("title = {}".format(self.title ))


    # locate the data block offset with the corresponding block name
    def locate_block(self, block_name, show_block=False):
        if (block_name in self.block_info) and (self.block_info[block_name] is not None):
            ...
            while (curr_name != block_name) and (curr_name != ""):
                ...
                next_pos += size
                new_pos=self.file_obj.seek(next_pos)
                if new_pos != next_pos or new_pos>=self.file_size-1:
                    #print("Reached to the end ...")
                    break
                if show_block:
                    print("Block name={}, starting pos={}, size={}".format(curr_name, curr_pos, size))
            # found the id
    ...
    def get_spectra(self, start=0, end=-1):
        ...
        n_row = end - start + 1
        pos=self.file_obj.seek(pos_start)
